...
hourly_stats: "id, userId, metric, year"
  ...
  const log$f = createLogger("Settings");
...
load() {
      try {
        ...
        log$f.error("Error loading settings, using defaults", { error: e });
...
setSnapToEdge(enabled) {
  ...
  const log$e = createLogger("RateLimiter");
...
setBackoff(until) {
    ...
    async processReportQueue() {
      ...
      try {
        ...
        log$e.error("Report fetch failed", { url: task.url, error: e });
        task.reject(e);
      } finally {
...
getTabCount() {
  ...
  const log$d = createLogger("ProfileContext");
...
constructor() {
      try {
        ...
        log$d.error("Context init failed", { error: e });
...
getTargetUserInfo() {
      ...
      try {
        ...
        if (!name) return null;
        if (!id) {
          log$d.warn("User ID not found, functionality may be limited (Notes)");
        }
        return {
          name,
        ...
        log$d.warn("User info extraction error", { error: e });
        return null;
      }
    }
isValidProfile() {
  ...
  const log$c = createLogger("DataManager");
  class DataManager {
    baseUrl;
...
async fetchPostDetails(postId) {
      try {
        ...
        log$c.warn(`Failed to fetch post details for post ${postId}`, { error: e });
      }
      return null;
    }
async getStats(key, userId) {
      try {
        ...
        log$c.warn("Failed to load stats cache", { error: e });
        return null;
      }
    }
async saveStats(key, userId, data) {
      try {
        ...
        log$c.warn("Failed to save stats cache", { error: e });
      }
    }
async getGrassSettings(userId) {
      ...
      try {
        ...
        log$c.warn("Failed to load grass settings", { error: e });
        return null;
      }
    }
async saveGrassSettings(userId, settings) {
      ...
      try {
        ...
        log$c.warn("Failed to save grass settings", { error: e });
      }
    }
async checkYearCompletion(userId, metric, year) {
      ...
      try {
        ...
        log$c.warn("Failed to check year completion status", { error: e });
        return false;
      }
    }
async markYearComplete(userId, metric, year) {
      try {
        ...
        log$c.warn("Failed to mark year complete", { error: e });
      }
    }
async getMetricData(metric, userInfo, year, onProgress = null) {
      try {
        ...
        if (!isYearCompleteCache && metric === "uploads" && year < ( new Date()).getFullYear()) {
          try {
            ...
            if (remoteCount !== localCount) {
              log$c.warn(`Data mismatch detected for ${year}, forcing full sync`, {
                remoteCount,
                localCount
              });
          ...
          if (!isYearCompleteCache) {
            ...
            items.forEach((item) => {
              ...
              if (userInfo.id && item[idKey] && String(item[idKey]) !== String(userInfo.id)) {
                log$c.warn("ID mismatch, skipping item", {
        ...
        log$c.error("Metric data fetch failed", { error: e });
        throw e;
      }
    }
async clearCache(_metric, userInfo) {
      try {
        ...
        log$c.error("Clear cache failed", { error: e });
        return false;
      }
    }
async fetchAllPages(endpoint, params, stopDate = null, dateKey = "created_at", direction = "desc", onProgress = null, isDelta = false) {
      ...
      while (true) {
        ...
        for (let i = 0; i < batchSize; i++) {
          ...
          promises.push(
            fetchTask().catch((e) => {
              log$c.error(`Critical fetch error on page ${currentPage}`, {
                page: currentPage,
                error: e
              });
              throw e;
        ...
        page += fetchedBatch;
        if (page > 1e3) {
          log$c.warn("Hit safety page limit of 1000, stopping fetch");
          break;
        }
...
async fetchPromotionDate(userName) {
      try {
        ...
        log$c.warn("Failed to fetch promotion date", { error: e });
        return null;
      }
    }
async getCacheStats() {
      ...
      try {
        ...
        log$c.warn("Failed to get IndexedDB stats", { error: e });
      }
      let lsCount = 0;
      let lsSize = 0;
...
async revalidateCurrentYearCache(userId, normalizedName) {
      ...
      for (const metric of ["uploads", "approvals", "notes"]) {
        try {
          ...
          log$c.warn(`v924 revalidation: ${metric} check failed, will retry`, {
            error: e
          });
          return;
        }
      ...
      if (anyMismatch) {
        log$c.info("v924 revalidation: cleared stale data, will refetch");
      }
      try {
  ...
  const log$b = createLogger("ApprovalPopover");
  async function showApprovalsDetail(db, dateStr, userId, event, fetchPostDetails) {
    ...
    if (!detail) {
      log$b.warn(`No entry found in approvals_detail for ID: ${detailId}`);
      return;
    }
    if (!detail.post_list || detail.post_list.length === 0) {
      log$b.warn("Entry found but post_list is empty", { detailId });
      return;
    }
  ...
  const log$a = createLogger("GraphRenderer");
  class GraphRenderer {
    containerId;
    cal;
    settingsManager;
...
async injectSkeleton(dataManager, userId) {
      ...
      if (!stats) {
        log$a.error("Injection point not found");
        return false;
      }
...
async renderGraph(dataMap, year, metric, userInfo, availableYears, onYearChange, onRefresh, skipScroll = false) {
      ...
      if (win.cal && typeof win.cal.destroy === "function") {
        try {
          ...
          log$a.warn("Failed to destroy previous CalHeatmap instance", { error: e });
        }
      }
      ...
      win.cal.paint(buildPaintConfig()).then(() => {
        ...
        const onThemeChange = () => {
          try {
            ...
            log$a.debug("CalHeatmap re-paint failed", { error: e });
          }
        ...
        log$a.error("CalHeatmap render failed", { error: err });
        this.updateSummaryGrid(hourlyData, metric);
      });
    }
...
async showApprovalsDetail(dateStr, userId, event) {
  ...
  const log$9 = createLogger("GrassApp");
  class GrassApp {
    db;
    settings;
    context;
...
async run() {
      ...
      await dataManager.revalidateCurrentYearCache(userId, normalizedName).catch((e) => {
        log$9.warn("Cache revalidation failed, continuing normally", { error: e });
      });
...
async () => {
          ...
          log$9.error("Failed to render grass graph", { error: e });
...
swrStats(
        ...
        perfLogger.wrap(
          "render.fetchData.timelineMilestones",
          () => dataManager.getTimelineMilestones(user)
        ),
        perfLogger.wrap(
          "render.fetchData.tagCloudGeneral",
          () => dataManager.getTagCloudData(user, 0)
        ),
        perfLogger.wrap(
          "render.fetchData.userStats",
          () => dataManager.getUserStats(user)
        ),
        perfLogger.wrap(
          "render.fetchData.needsBackfill",
  ...
  function renderTagCloudWidget(container2, options) {
    ...
    if (isTouch) {
      tapController = createTwoStepTap({
        ...
        onReset: () => {
          ...
          d3__namespace.select(cloudContainer).selectAll("text").style("opacity", 1);
        },
        resetOnScroll: true,
        isEqual: (a, b) => a === b
      });
    ...
    const createCloudSvg = (placedWords, width, color, startOpacity) => {
      ...
      g.selectAll("text").data(placedWords).join("text").attr("class", "di-tag-cloud-word").style("font-size", (d) => `${d.size}px`).style("font-weight", (d) => d.bold ? "700" : "500").style("font-family", "sans-serif").style("fill", color).attr("text-anchor", "middle").attr(
        ...
        if (tapController) {
          const isSecondTap = tapController.tap(d.tagName);
          if (isSecondTap) {
            const query2 = `user:${userName} ${d.tagName}`;
            window.open(`/posts?tags=${encodeURIComponent(query2)}`, "_blank");
            cloudTooltip.style.opacity = "0";
            g.selectAll("text").style("opacity", 1);
            d3__namespace.select(this).style("font-size", `${d.size}px`);
          } else {
        ...
        await Promise.all(
          topTags.map(async (obj) => {
            try {
              ...
              log$2.debug("Failed to fetch combined tag count", { error: e });
            }
          })
        );
        const finalTags = [];
...
async _fetchLargeTag(meta, initialStats, runDelta, baseData, startDate, initialPosts) {
      ...
      const [resolvedRankings, historyData, milestones, first100StatsRaw] = heavyResults;
