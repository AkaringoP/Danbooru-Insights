...
midnight: {
  ...
  const log$l = createLogger("Database");
...
hourly_stats: "id, userId, metric, year"
  ...
  const log$k = createLogger("Settings");
...
load() {
      try {
        ...
        log$k.error("Error loading settings, using defaults", { error: e });
...
setSnapToEdge(enabled) {
  ...
  const log$j = createLogger("RateLimiter");
...
setBackoff(until) {
    ...
    async processReportQueue() {
      ...
      try {
        ...
        log$j.error("Report fetch failed", { url: task.url, error: e });
        task.reject(e);
      } finally {
...
getTabCount() {
  ...
  const log$i = createLogger("ProfileContext");
...
constructor() {
      try {
        ...
        log$i.error("Context init failed", { error: e });
...
getTargetUserInfo() {
      ...
      try {
        ...
        if (!name) return null;
        if (!id) {
          log$i.warn("User ID not found, functionality may be limited (Notes)");
        }
        return {
          name,
        ...
        log$i.warn("User info extraction error", { error: e });
        return null;
      }
    }
isValidProfile() {
  ...
  const log$h = createLogger("DataManager");
...
async fetchPostDetails(postId) {
      try {
        ...
        log$h.warn(`Failed to fetch post details for post ${postId}`, { error: e });
      }
      return null;
    }
async getStats(key, userId, maxAgeMs) {
      try {
        ...
        log$h.warn("Failed to load stats cache", { error: e });
        return null;
      }
    }
async saveStats(key, userId, data) {
      try {
        ...
        log$h.warn("Failed to save stats cache", { error: e });
      }
    }
async getGrassSettings(userId) {
      ...
      try {
        ...
        log$h.warn("Failed to load grass settings", { error: e });
        return null;
      }
    }
async saveGrassSettings(userId, settings) {
      ...
      try {
        ...
        log$h.warn("Failed to save grass settings", { error: e });
      }
    }
async checkYearCompletion(userId, metric, year) {
      ...
      try {
        ...
        log$h.warn("Failed to check year completion status", { error: e });
        return false;
      }
    }
async sumDailyCounts(metric, userId, startDate, endDate) {
      ...
      try {
        ...
        log$h.warn("Failed to sum daily counts", { error: e, metric });
        return 0;
      }
    }
async markYearComplete(userId, metric, year) {
      try {
        ...
        log$h.warn("Failed to mark year complete", { error: e });
      }
    }
async getMetricData(metric, userInfo, year, onProgress = null) {
      try {
        ...
        log$h.error("Metric data fetch failed", { error: e });
        throw e;
      }
    }
...
async runUploadsIntegrityCheck(args) {
      ...
      try {
        ...
        log$h.warn(`Data mismatch detected for ${year}, forcing full sync`, {
          remoteCount,
          localCount
        });
        ...
        log$h.warn("Integrity check failed (Network/API), proceeding with cache", {
          error: e
        });
        return false;
      }
...
async fetchAndPersistYear(args) {
      ...
      items.forEach((item) => {
        ...
        if (userInfo.id && item[idKey] && String(item[idKey]) !== String(userInfo.id)) {
          log$h.warn("ID mismatch, skipping item", {
...
async clearCache(_metric, userInfo) {
      try {
        ...
        log$h.error("Clear cache failed", { error: e });
        return false;
      }
    }
...
async fetchAllPages(endpoint, params, stopDate = null, dateKey = "created_at", direction = "desc", onProgress = null, isDelta = false) {
      ...
      while (true) {
        ...
        for (let i = 0; i < batchSize; i++) {
          ...
          promises.push(
            fetchTask().catch((e) => {
              log$h.error(`Critical fetch error on page ${currentPage}`, {
                page: currentPage,
                error: e
              });
              throw e;
        ...
        page += fetchedBatch;
        if (page > 1e3) {
          log$h.warn("Hit safety page limit of 1000, stopping fetch");
          break;
        }
...
async fetchPromotionDate(userName) {
      try {
        ...
        log$h.warn("Failed to fetch promotion date", { error: e });
        return null;
      }
    }
async getCacheStats() {
      ...
      try {
        ...
        log$h.warn("Failed to get IndexedDB stats", { error: e });
      }
      let lsCount = 0;
      let lsSize = 0;
...
`;
  ...
  function nearestRank(sorted, percentile2) {
    const n = sorted.length;
    if (n === 0) return 0;
    const idx = Math.max(0, Math.ceil(percentile2 / 100 * n) - 1);
    return sorted[idx];
  }
...
get isTracking() {
  ...
  const log$g = createLogger("ApprovalPopover");
  async function showApprovalsDetail(db, dateStr, userId, event, fetchPostDetails) {
    ...
    if (!detail) {
      log$g.warn(`No entry found in approvals_detail for ID: ${detailId}`);
      return;
    }
    if (!detail.post_list || detail.post_list.length === 0) {
      log$g.warn("Entry found but post_list is empty", { detailId });
      return;
    }
  ...
  const log$f = createLogger("GrassPrevMonth");
  ...
  function readMemo(key) {
    try {
      ...
      log$f.debug("Could not read December memo", { error: e });
      return null;
    }
  }
  function writeMemo(key, value) {
    try {
      ...
      log$f.debug("Could not memoise December total", { error: e });
    }
  }
  async function countByPaging(dataManager, metric, user, year) {
    ...
    log$f.debug("December page walk hit its cap; skipping the delta", {
      metric,
      year
    });
    return null;
  ...
  async function fetchDecemberTotal(dataManager, user, metric, year) {
    try {
      ...
      log$f.debug("December total lookup failed", { error: e, metric, year });
      return null;
    }
  }
  ...
  const log$e = createLogger("GraphRenderer");
...
async injectSkeleton(dataManager, userId) {
      ...
      if (!stats) {
        log$e.error("Injection point not found");
        return false;
      }
...
async renderGraph(dataMap, year, metric, userInfo, availableYears, onYearChange, onRefresh, skipScroll = false) {
      ...
      if (win.cal && typeof win.cal.destroy === "function") {
        try {
          ...
          log$e.warn("Failed to destroy previous CalHeatmap instance", { error: e });
        }
      }
      ...
      win.cal.paint(makePaintConfig()).then(() => {
        ...
        const onThemeChange = () => {
          try {
            ...
            log$e.debug("CalHeatmap re-paint failed", { error: e });
          }
        ...
        log$e.error("CalHeatmap render failed", { error: err });
        this.updateSummaryGrid(hourlyData, metric);
      });
    }
...
async showApprovalsDetail(dateStr, userId, event) {
  ...
  const log$d = createLogger("GrassApp");
  class GrassApp {
    db;
    settings;
    context;
...
async () => {
          ...
          log$d.error("Failed to render grass graph", { error: e });
          if (!isCurrent()) return;
...
onClose: () => dismissSuggestion(userId),
        ...
        log$d.warn("Auto-tune suggestion check failed", { error: e });
      }
    }
...
onClose: () => dismissSuggestion(userId),
        ...
        log$d.warn("Scheduled auto-tune check failed", { error: e });
        return false;
      }
    }
...
dumpStats() {
  ...
  const log$c = createLogger("Quota");
  ...
  async function bulkPutSafe(table, records, evictor) {
    if (Math.random() < SAMPLING_RATE) {
      ...
      if (snapshot.available && snapshot.ratio > QUOTA_PRESSURE_THRESHOLD) {
        log$c.warn("Storage quota high — pre-emptive eviction", {
        ...
        try {
          ...
          log$c.warn("Pre-emptive eviction failed", { error: unwrapAbortError(e) });
        }
      }
    }
    try {
      ...
      if (!isQuotaExceeded(e)) {
        log$c.error("bulkPut failed", {
          error: unwrapAbortError(e),
          records: records.length
        });
        throw e;
      }
      log$c.warn("QuotaExceededError on bulkPut — evicting and retrying", {
        records: records.length
      });
      await evictor();
    }
  ...
  async function requestPersistence() {
    ...
    try {
      ...
      if (granted) {
        log$c.info("Persistent storage granted");
        try {
      ...
      log$c.warn("navigator.storage.persist threw", {
        error: unwrapAbortError(e)
      });
      return false;
    }
  ...
  async function evictOldestNonCurrentUser(db, currentUserId) {
    ...
    try {
      ...
      if (oldestUid === null) {
        log$c.info("No non-current user available for eviction");
        return null;
      }
      ...
      log$c.info("Evicted oldest non-current user", {
        uid: oldestUid,
        lastSyncMs: Number.isFinite(oldestTime) ? oldestTime : null
      });
      return oldestUid;
    } catch (e) {
      log$c.warn("Eviction failed", { error: unwrapAbortError(e) });
      return null;
    }
  }
  const log$b = createLogger("GlobalTagStats");
  ...
  function writeCache(key, value) {
    try {
      ...
      log$b.debug("Failed to write cache", { key, error: e });
    }
  }
  async function getGlobalTotalPosts(rateLimiter) {
    ...
    try {
      ...
      log$b.debug("Failed to fetch global total posts", { error: e });
      return 0;
    }
  }
  async function getGlobalTopGeneralTags(rateLimiter) {
    ...
    try {
      ...
      log$b.debug("Failed to fetch global top-50 general tags", { error: e });
      return new Map();
    }
  }
  ...
  const log$a = createLogger("SubTagResolver");
  ...
  async function fetchSubTagsForParents(rateLimiter, db, parents) {
    ...
    if (table) {
      ...
      try {
        ...
        log$a.warn("Failed to read implication cache, refetching all", { error: e });
...
isTopLevel: false,
      ...
      try {
        ...
        log$a.warn("Failed to write sub-tag cache (continuing)", { error: e });
      }
    }
    return result;
  }
  ...
  async function fetchInChunks(rateLimiter, parents) {
    ...
    for (let i = 0; i < parents.length; i += CHUNK_SIZE) {
      ...
      try {
        ...
        log$a.warn("Sub-tag chunk fetch failed (leaving empty)", {
...
rating: post.rating ?? "",
  ...
  const log$9 = createLogger("Analytics");
...
async fetchThumbnailWithRetry(tags, retries = 3, delay = 2e3) {
      ...
      for (let i = 0; i < retries; i++) {
        try {
          ...
          if (i === retries - 1) {
            log$9.warn(`Failed thumb fetch after ${retries} tries`, {
              tags,
              error: e
            });
            return "";
    ...
    async getMilestones(userInfo, isNsfwEnabled = false, customStep = "auto", forceRefresh = false) {
      ...
      if (missingIds.length > 0) {
        try {
              ...
              fetchedItems.forEach((item) => {
                ...
                if (local) {
                  ...
                  this.db.posts.update(local.id, {
                    ...
                    (e) => log$9.error("Failed to update post in DB", {
                      postId: local["id"],
                      error: e
                    })
                  );
          ...
          log$9.warn("Failed to fetch missing milestone thumbnails", { error: e });
        }
      }
...
async getStatusDistribution(userInfo, startDate = null, forceRefresh = false) {
      ...
      const tasks = statuses.map(async (status) => {
        try {
          ...
          log$9.warn("Failed to fetch count for status", { status, error: e });
          return {
            name: status,
            count: 0,
            label: status.charAt(0).toUpperCase() + status.slice(1)
...
async getRatingDistribution(userInfo, startDate = null, forceRefresh = false) {
      ...
      const tasks = ratings.map(async (rating) => {
        try {
          ...
          log$9.warn("Failed to fetch count for rating", { rating, error: e });
      ...
      try {
        ...
        log$9.error("Failed to fetch rating distribution", { error: e });
        return [];
      }
    }
async getTagCloudData(userInfo, categoryId, forceRefresh = false) {
      ...
      try {
        ...
        log$9.debug("Failed to fetch tag cloud data", { error: e });
        return [];
      }
    }
...
async getCreatedTags(userInfo, onProgress) {
      ...
      try {
        ...
        log$9.debug("Failed to fetch created tags", { error: e });
        return [];
      }
    }
async getCharacterDistribution(userInfo, forceRefresh = false, reportSubStatus = null) {
      ...
      try {
        ...
        await perfLogger.wrap(
          ...
          () => this.mapConcurrent(preItems, 6, async (obj) => {
            ...
            try {
              ...
              log$9.debug("Failed to fetch user tag count", { error: _e });
            }
          }),
        ...
        log$9.warn("Failed to fetch character distribution", { error: e });
        return [];
      }
    }
async getCopyrightDistribution(userInfo, forceRefresh = false, reportSubStatus = null) {
      ...
      try {
        ...
        log$9.warn("Failed to fetch copyright distribution", { error: e });
        return [];
      }
    }
...
async attachSubTagBreakdowns(top10, allUserTags, countQueryPrefix) {
      ...
      await this.mapConcurrent([...subsToFetch], 5, async (sub) => {
        try {
          ...
          log$9.debug("Failed to fetch sub-tag count", { sub, error: e });
        }
      });
      ...
      try {
        ...
        await this.mapConcurrent(preItems, 6, async (obj) => {
          ...
          try {
            ...
            log$9.warn("Count fetch failed for fav copyright tag", {
              tagName: obj.tagName,
              error: e
            });
          }
        ...
        try {
          ...
          log$9.debug("Failed to fetch total fav count", { error: e });
        }
        const sumCount = top10.reduce(
          (acc, curr) => acc + curr.count,
          0
        ...
        log$9.warn("Failed to fetch fav copyright distribution", { error: e });
        return [];
      }
    }
async getTopPostsByType(userInfo, forceRefresh = false) {
      ...
      const fetchTop = async (ratingTag, extraQuery = "") => {
        try {
          ...
          log$9.warn("Failed to fetch top post for rating", { ratingTag, error: e2 });
        }
        return null;
      };
...
async getRecentPopularPosts(userInfo, forceRefresh = false) {
      ...
      const fetchTop = async (ratingTag) => {
        try {
          ...
          log$9.warn("Failed to fetch recent top post", { ratingTag, error: e });
        }
        return null;
      };
...
async getRecentPostsPreview(userInfo, limit = 20) {
      ...
      try {
        ...
        log$9.warn("Failed to fetch recent posts preview", {
          user: userInfo.name,
          error: e
        });
        return [];
      ...
      const flags = await this.mapConcurrent(
        ...
        async (postId) => {
          try {
            ...
            log$9.warn("Abandoned-gap lookup failed", { postId, error: e });
            return null;
          }
        }
      );
...
async fetchUploadActivity(d, userId, limit) {
      try {
        ...
        log$9.warn("Activity feed fetch failed", { type: "upload", error: e });
        return [];
      }
    }
async fetchCommentActivity(d, userId, limit) {
      try {
        ...
        log$9.warn("Activity feed fetch failed", { type: "comment", error: e });
        return [];
      }
    }
async fetchPostBadnessMeta(postIds) {
      ...
      try {
        ...
        log$9.warn("Post badness lookup failed", { count: unique.length, error: e });
      }
      return map;
    }
async fetchActivityType(d, userId, limit) {
      try {
        ...
        log$9.warn("Activity feed fetch failed", { type: d.type, error: e });
        return [];
      }
    }
async fetchCommentaryActivity(d, userId, limit) {
      try {
        ...
        log$9.warn("Activity feed fetch failed", { type: "commentary", error: e });
        return [];
      }
    }
async fetchPostUploadTimes(postIds) {
      ...
      try {
        ...
        log$9.warn("Post upload-time lookup failed", {
          count: unique.length,
          error: e
        });
      }
...
async getRandomPosts(userInfo) {
      ...
      const fetchRandom = async (ratingTag) => {
        try {
          ...
          log$9.warn("Failed to fetch random post", { ratingTag, error: e });
        }
        return null;
      };
      const [sfw, nsfw] = await Promise.all([
...
recordBackfillFailure(uploaderId) {
      ...
      log$9.warn("Backfill failure recorded", {
        uploaderId,
        ...
        try {
          ...
          if (!resp.ok) {
            if (shouldCountHttpAsFailure(resp.status)) {
              log$9.warn("Backfill HTTP error — pausing backfill", {
                status: resp.status
              });
          ...
          log$9.warn("Backfill fetch failed", { error: e });
          this.recordBackfillFailure(uploaderId);
          return;
        }
...
async getUserStats(userInfo, force = false) {
      ...
      const fetchCount = (tagQuery) => this.fetchRemoteCount(tagQuery).catch((e) => {
        log$9.warn("Count query failed for user stats", { tagQuery, error: e });
        return 0;
      });
...
async getPromotionHistory(userInfo) {
      ...
      try {
        ...
        log$9.error("Failed to fetch promotion history", { error: e });
        return [];
      }
    }
async getLevelChangeHistory(userInfo, forceRefresh = false) {
      ...
      try {
        ...
        log$9.warn("Failed to fetch level change history", { error: e });
        return [];
      }
    }
      ...
      await this.mapConcurrent(
        ...
        async (item) => {
          ...
          try {
            ...
            log$9.debug("Failed to fetch commentary count", { error: e });
          }
        }
      );
      ...
      await this.mapConcurrent(
        ...
        async (item) => {
          ...
          try {
            ...
            log$9.debug("Failed to fetch translation count", { error: e });
          }
        }
      );
      const filtered = results.filter((r) => r.count > 0);
      ...
      await this.mapConcurrent(
        ...
        async (item) => {
          ...
          try {
            ...
            log$9.debug("Failed to fetch gender count", { error: e });
          }
        }
      );
      const filtered = results.filter((r) => r.count > 0);
...
async getBreastsDistribution(userInfo, forceRefresh = false, reportSubStatus = null) {
      ...
      await this.mapConcurrent(results, 3, async (obj) => {
        ...
        try {
          ...
          log$9.debug("Failed to fetch breasts count", { error: e });
        }
      });
...
async getHairLengthDistribution(userInfo, forceRefresh = false, reportSubStatus = null) {
      ...
      await this.mapConcurrent(results, 3, async (obj) => {
        ...
        try {
          ...
          log$9.debug("Failed to fetch count", { error: e });
        }
      });
      const filtered = results.filter((r) => r.count > 0).sort((a, b) => b.count - a.count);
      await this.carryOverThumbs(cacheKey, uploaderId, filtered);
...
async fetchTotalPostCount(userInfo) {
      try {
        ...
        log$9.warn("Counts API failed", { error: e });
      }
      try {
        ...
        log$9.debug("Failed to fetch user profile", { error: _e2 });
      }
      try {
        ...
        log$9.debug("Failed to parse DOM stats", { error: _e3 });
      }
      return 0;
    }
...
async pruneGhostPosts(uploaderId, startId, seenIds) {
      ...
      log$9.debug("Pruned remotely-deleted posts", {
        uploaderId,
        startId,
...
async cleanupStaleData(currentUserId) {
      ...
      try {
        ...
        log$9.warn("Stale data cleanup failed", { error: e });
      }
    }
...
perfLogger.wrap(
        ...
        log$9.warn("Failed to refresh critical stats", { error: e });
      } finally {
...
distributionRevalidators: distributionsSwr.revalidators,
  ...
  const log$8 = createLogger("UserAnalyticsCharts");
...
onHide: () => subRowHighlight.reset(),
  ...
  `;
    const loadTab = async (tabName) => {
      ...
      try {
        ...
        log$8.error("Failed to load pie chart data", { error: e });
    ...
    if (refreshBtn) {
      refreshBtn.onclick = async (e) => {
        ...
        try {
          ...
          log$8.error("Failed to refresh random post", { error: err });
        } finally {
  ...
  const log$7 = createLogger("Scatter");
    ...
    options.runBackfill((cur, total) => {
      ...
      if (options.refreshScatterData) {
        try {
          ...
          log$7.warn("Refresh after backfill failed", { error: e });
        }
      }
    }).catch((e) => {
      log$7.warn("Backfill failed", { error: e });
      state2.backfillInProgress = false;
  ...
  const log$6 = createLogger("TagCloud");
  ...
  function renderTagCloudWidget(container2, options) {
    ...
    const loadTab = async (categoryId, crossfade) => {
      ...
      try {
        ...
        log$6.debug("Tag cloud tab load failed", { error: e });
        if (currentTab === categoryId) {
  ...
  const log$5 = createLogger("WidgetLocked");
