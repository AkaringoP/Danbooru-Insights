...
midnight: {
  ...
  const log$f = createLogger("Database");
...
hourly_stats: "id, userId, metric, year"
  ...
  const log$e = createLogger("Settings");
...
load() {
      try {
        ...
        log$e.error("Error loading settings, using defaults", { error: e });
...
setSnapToEdge(enabled) {
  ...
  const log$d = createLogger("RateLimiter");
...
setBackoff(until) {
    ...
    async processReportQueue() {
      ...
      try {
        ...
        log$d.error("Report fetch failed", { url: task.url, error: e });
        task.reject(e);
      } finally {
...
getTabCount() {
  ...
  const log$c = createLogger("ProfileContext");
...
constructor() {
      try {
        ...
        log$c.error("Context init failed", { error: e });
...
getTargetUserInfo() {
      ...
      try {
        ...
        if (!name) return null;
        if (!id) {
          log$c.warn("User ID not found, functionality may be limited (Notes)");
        }
        return {
          name,
        ...
        log$c.warn("User info extraction error", { error: e });
        return null;
      }
    }
isValidProfile() {
  ...
  const log$b = createLogger("DataManager");
  class DataManager {
    baseUrl;
...
async fetchPostDetails(postId) {
      try {
        ...
        log$b.warn(`Failed to fetch post details for post ${postId}`, { error: e });
      }
      return null;
    }
async getStats(key, userId) {
      try {
        ...
        log$b.warn("Failed to load stats cache", { error: e });
        return null;
      }
    }
async saveStats(key, userId, data) {
      try {
        ...
        log$b.warn("Failed to save stats cache", { error: e });
      }
    }
async getGrassSettings(userId) {
      ...
      try {
        ...
        log$b.warn("Failed to load grass settings", { error: e });
        return null;
      }
    }
async saveGrassSettings(userId, settings) {
      ...
      try {
        ...
        log$b.warn("Failed to save grass settings", { error: e });
      }
    }
async checkYearCompletion(userId, metric, year) {
      ...
      try {
        ...
        log$b.warn("Failed to check year completion status", { error: e });
        return false;
      }
    }
async markYearComplete(userId, metric, year) {
      try {
        ...
        log$b.warn("Failed to mark year complete", { error: e });
      }
    }
async getMetricData(metric, userInfo, year, onProgress = null) {
      try {
        ...
        if (!isYearCompleteCache && metric === "uploads" && year < ( new Date()).getFullYear()) {
          try {
            ...
            if (remoteCount !== localCount) {
              log$b.warn(`Data mismatch detected for ${year}, forcing full sync`, {
                remoteCount,
                localCount
              });
          ...
          if (!isYearCompleteCache) {
            ...
            items.forEach((item) => {
              ...
              if (userInfo.id && item[idKey] && String(item[idKey]) !== String(userInfo.id)) {
                log$b.warn("ID mismatch, skipping item", {
        ...
        log$b.error("Metric data fetch failed", { error: e });
        throw e;
      }
    }
async clearCache(_metric, userInfo) {
      try {
        ...
        log$b.error("Clear cache failed", { error: e });
        return false;
      }
    }
async fetchAllPages(endpoint, params, stopDate = null, dateKey = "created_at", direction = "desc", onProgress = null, isDelta = false) {
      ...
      while (true) {
        ...
        for (let i = 0; i < batchSize; i++) {
          ...
          promises.push(
            fetchTask().catch((e) => {
              log$b.error(`Critical fetch error on page ${currentPage}`, {
                page: currentPage,
                error: e
              });
              throw e;
        ...
        page += fetchedBatch;
        if (page > 1e3) {
          log$b.warn("Hit safety page limit of 1000, stopping fetch");
          break;
        }
...
async fetchPromotionDate(userName) {
      try {
        ...
        log$b.warn("Failed to fetch promotion date", { error: e });
        return null;
      }
    }
async getCacheStats() {
      ...
      try {
        ...
        log$b.warn("Failed to get IndexedDB stats", { error: e });
      }
      let lsCount = 0;
      let lsSize = 0;
...
async revalidateCurrentYearCache(userId, normalizedName) {
      ...
      for (const metric of ["uploads", "approvals", "notes"]) {
        try {
          ...
          log$b.warn(`v924 revalidation: ${metric} check failed, will retry`, {
            error: e
          });
          return;
        }
  ...
  const log$a = createLogger("ApprovalPopover");
  async function showApprovalsDetail(db, dateStr, userId, event, fetchPostDetails) {
    ...
    if (!detail) {
      log$a.warn(`No entry found in approvals_detail for ID: ${detailId}`);
      return;
    }
    if (!detail.post_list || detail.post_list.length === 0) {
      log$a.warn("Entry found but post_list is empty", { detailId });
      return;
    }
  ...
  const log$9 = createLogger("GraphRenderer");
  class GraphRenderer {
    containerId;
    cal;
    settingsManager;
...
async injectSkeleton(dataManager, userId) {
      ...
      if (!stats) {
        log$9.error("Injection point not found");
        return false;
      }
...
async renderGraph(dataMap, year, metric, userInfo, availableYears, onYearChange, onRefresh, skipScroll = false) {
      ...
      if (win.cal && typeof win.cal.destroy === "function") {
        try {
          ...
          log$9.warn("Failed to destroy previous CalHeatmap instance", { error: e });
        }
      }
...
case "notes":
      ...
      win.cal.paint(buildPaintConfig()).then(() => {
        ...
        log$9.error("CalHeatmap render failed", { error: err });
        this.updateSummaryGrid(hourlyData, metric);
      });
    }
...
async showApprovalsDetail(dateStr, userId, event) {
  ...
  const log$8 = createLogger("GrassApp");
  class GrassApp {
    db;
    settings;
    context;
...
async run() {
      ...
      await dataManager.revalidateCurrentYearCache(userId, normalizedName).catch((e) => {
        log$8.warn("Cache revalidation failed, continuing normally", { error: e });
      });
...
async () => {
          ...
          log$8.error("Failed to render grass graph", { error: e });
...
dumpStats() {
  ...
  const log$7 = createLogger("Quota");
  ...
  async function bulkPutSafe(table, records, evictor) {
    if (Math.random() < SAMPLING_RATE) {
      ...
      if (snapshot.available && snapshot.ratio > QUOTA_PRESSURE_THRESHOLD) {
        log$7.warn("Storage quota high — pre-emptive eviction", {
        ...
        try {
          ...
          log$7.warn("Pre-emptive eviction failed", { error: unwrapAbortError(e) });
        }
      }
    }
    try {
      ...
      if (!isQuotaExceeded(e)) {
        log$7.error("bulkPut failed", {
          error: unwrapAbortError(e),
          records: records.length
        });
        throw e;
      }
      log$7.warn("QuotaExceededError on bulkPut — evicting and retrying", {
        records: records.length
      });
      await evictor();
    }
  ...
  async function requestPersistence() {
    ...
    try {
      ...
      if (granted) {
        log$7.info("Persistent storage granted");
        try {
      ...
      log$7.warn("navigator.storage.persist threw", {
        error: unwrapAbortError(e)
      });
      return false;
    }
  ...
  async function evictOldestNonCurrentUser(db, currentUserId) {
    ...
    try {
      ...
      if (oldestUid === null) {
        log$7.info("No non-current user available for eviction");
        return null;
      }
      ...
      log$7.info("Evicted oldest non-current user", {
        uid: oldestUid,
        lastSyncMs: Number.isFinite(oldestTime) ? oldestTime : null
      });
      return oldestUid;
    } catch (e) {
      log$7.warn("Eviction failed", { error: unwrapAbortError(e) });
      return null;
    }
  }
  const log$6 = createLogger("Analytics");
...
async fetchThumbnailWithRetry(tags, retries = 3, delay = 2e3) {
      ...
      for (let i = 0; i < retries; i++) {
        try {
          ...
          if (i === retries - 1) {
            log$6.warn(`Failed thumb fetch after ${retries} tries`, {
              tags,
              error: e
            });
            return "";
    ...
    async getMilestones(userInfo, isNsfwEnabled = false, customStep = "auto", forceRefresh = false) {
      ...
      if (missingIds.length > 0) {
        try {
              ...
              fetchedItems.forEach((item) => {
                ...
                if (local) {
                  ...
                  this.db.posts.update(local.id, {
                    ...
                    (e) => log$6.error("Failed to update post in DB", {
                      postId: local["id"],
                      error: e
                    })
                  );
          ...
          log$6.warn("Failed to fetch missing milestone thumbnails", { error: e });
        }
      }
...
async getStatusDistribution(userInfo, startDate = null, forceRefresh = false) {
      ...
      const tasks = statuses.map(async (status) => {
        try {
          ...
          log$6.warn("Failed to fetch count for status", { status, error: e });
          return {
            name: status,
            count: 0,
            label: status.charAt(0).toUpperCase() + status.slice(1)
...
async getRatingDistribution(userInfo, startDate = null, forceRefresh = false) {
      ...
      const tasks = ratings.map(async (rating) => {
        try {
          ...
          log$6.warn("Failed to fetch count for rating", { rating, error: e });
      ...
      try {
        ...
        log$6.error("Failed to fetch rating distribution", { error: e });
        return [];
      }
    }
...
async getCharacterDistribution(userInfo, forceRefresh = false, reportSubStatus = null) {
      ...
      try {
        ...
        await perfLogger.wrap(
          ...
          () => this.mapConcurrent(top10, 3, async (obj) => {
            ...
            try {
              ...
              log$6.debug("Failed to fetch user tag count", { error: _e });
            }
            delete obj._item;
          }),
        ...
        log$6.warn("Failed to fetch character distribution", { error: e });
        return [];
      }
    }
async getCopyrightDistribution(userInfo, forceRefresh = false, reportSubStatus = null) {
      ...
      try {
        ...
        log$6.warn("Failed to fetch copyright distribution", { error: e });
        return [];
      }
    }
...
isOther: false,
        ...
        await this.mapConcurrent(top10, 3, async (obj) => {
          ...
          try {
            ...
            log$6.warn("Count fetch failed for fav copyright tag", {
              tagName: obj.tagName,
              error: e
            });
          }
        ...
        log$6.warn("Failed to fetch fav copyright distribution", { error: e });
        return [];
      }
    }
async getTopPostsByType(userInfo, forceRefresh = false) {
      ...
      const fetchTop = async (ratingTag, extraQuery = "") => {
        try {
          ...
          log$6.warn("Failed to fetch top post for rating", { ratingTag, error: e2 });
        }
        return null;
      };
...
async getRecentPopularPosts(userInfo, forceRefresh = false) {
      ...
      const fetchTop = async (ratingTag) => {
        try {
          ...
          log$6.warn("Failed to fetch recent top post", { ratingTag, error: e });
        }
        return null;
      };
...
async getRandomPosts(userInfo) {
      ...
      const fetchRandom = async (ratingTag) => {
        try {
          ...
          log$6.warn("Failed to fetch random post", { ratingTag, error: e });
        }
        return null;
      };
      const [sfw, nsfw] = await Promise.all([
...
async getTopScorePost(userInfo, filterMode = "sfw") {
      ...
      try {
        ...
        log$6.warn("Failed to fetch top post details", {
          postId: topLocal.id,
          error: e
        });
      }
...
recordBackfillFailure(uploaderId) {
      ...
      log$6.warn("Backfill failure recorded", {
        uploaderId,
        ...
        try {
          ...
          if (!resp.ok) {
            if (shouldCountHttpAsFailure(resp.status)) {
              log$6.warn("Backfill HTTP error — pausing backfill", {
                status: resp.status
              });
          ...
          log$6.warn("Backfill fetch failed", { error: e });
          this.recordBackfillFailure(uploaderId);
          return;
        }
...
async getUserStats(userInfo, force = false) {
      ...
      const fetchCount = async (tagQuery) => {
        try {
          ...
          log$6.warn("Count query failed for user stats", { tagQuery, error: e });
          return 0;
        }
      };
...
async getPromotionHistory(userInfo) {
      ...
      try {
        ...
        log$6.error("Failed to fetch promotion history", { error: e });
        return [];
      }
    }
async getLevelChangeHistory(userInfo, forceRefresh = false) {
      ...
      try {
        ...
        log$6.warn("Failed to fetch level change history", { error: e });
        return [];
      }
    }
...
async getTotalPostCount(userInfo) {
      ...
      try {
        ...
        log$6.warn("Counts API failed", { error: e });
      }
      try {
...
async syncAllPosts(userInfo, onProgress) {
      if (!userInfo.id) {
        log$6.error("User ID required for sync");
        return;
      }
      const uploaderId = parseInt(userInfo.id ?? "0");
      if (AnalyticsDataManager.isGlobalSyncing) {
        log$6.warn("Sync already in progress");
        return;
      }
...
async cleanupStaleData(currentUserId) {
      ...
      try {
        ...
        log$6.warn("Stale data cleanup failed", { error: e });
      }
    }
...
...isFullSync ? [
        ...
        log$6.warn("Failed to refresh stats", { error: e });
      } finally {
...
swrStats(
  ...
  const log$5 = createLogger("UserAnalyticsCharts");
  function renderPieWidget(container2, distributions, initialNsfwEnabled, dataManager, context, firstUploadDate) {
    ...
    const renderPieContent = () => {
      ...
      const processedData = data.map((d, i) => {
        if ([
        ...
        ].includes(currentPieTab)) {
          {
            ...
            return {
              value: d.count,
              label: currentPieTab === "rating" ? ratingLabels[dFlexible.rating] || dFlexible.rating || "" : dFlexible.label || d.name || "",
              color: currentPieTab === "rating" ? ratingColors[dFlexible.rating] || "#999" : currentPieTab === "hair_color" && d.color ? d.color : d.color || (d.isOther ? "#bdbdbd" : palette[i % palette.length]),
              details: d
            };
          }
        } else {
      ...
      ).attr("stroke", "var(--di-chart-bg, #fff)").style("stroke-width", "1px").on("mouseover", function(_event, d) {
      ...
      if (isTouch) {
        ...
        svg.selectAll("path.danbooru-grass-pie-path").on("touchstart", (event) => {
          handleSliceTouch(event);
        }).on("touchmove", (event) => {
          handleSliceTouch(event);
        });
        tooltip.on("click", () => {
        ...
        const listHtml = processedData.map((d) => {
          ...
          return `
               <div style="display:flex; align-items:center; font-size:0.85em; margin-bottom:5px;">
                  ...
                  ${d.details.isOther ? `<div style="color:var(--di-text-secondary, #666); width:90px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="${d.label}">${d.label}</div>` : `<a href="${targetUrl}" target="_blank" class="di-hover-underline" style="color:var(--di-text-secondary, #666); width:90px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; text-decoration:none;" title="${d.label}">${d.label}</a>`}
  ...
  `;
    const loadTab = async (tabName) => {
      ...
      try {
        ...
        log$5.error("Failed to load pie chart data", { error: e });
    ...
    container2.addEventListener("click", (e) => {
      if (e.target.classList.contains("di-pie-tab")) {
        ...
        if (mode && currentPieTab !== mode) {
          ...
          const minBlurMs = 380;
          const minBlur = new Promise(
            (resolve) => setTimeout(resolve, minBlurMs)
          );
    ...
    if (refreshBtn) {
      refreshBtn.onclick = async (e) => {
        ...
        try {
          ...
          log$5.error("Failed to refresh random post", { error: err });
        } finally {
  ...
  const log$4 = createLogger("Scatter");
  ...
  function renderScatterCanvas(state, dom, scatterData, context, levelChanges, options) {
  ...
  function wireModeToggle(state, dom, rerender) {
  ...
  function wireYearReset(state, dom, rerender) {
    ...
    options.runBackfill((cur, total) => {
      ...
      if (options.refreshScatterData) {
        try {
          ...
          log$4.warn("Refresh after backfill failed", { error: e });
        }
      }
    }).catch((e) => {
      log$4.warn("Backfill failed", { error: e });
      state.backfillInProgress = false;
  ...
  function wireYearZoom(state, dom, rerender) {
  ...
  function wireDragSelection(state, dom, scatterData, showPopover) {
    const isTouch = isTouchDevice();
