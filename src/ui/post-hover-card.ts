...
const buildCardHtml = (post: PostDetails): string => {
  ...
  const tagLine = (icon: string, label: string, value: string) =>
    value
      ? `<div style="font-size:11px;color:var(--di-text-heading, #444);"><strong>${icon} ${label}:</strong> ${escapeHtml(value)}</div>`
      : '';

  const tagsBlock =
    artist || copyright || character
      ? `<div style="margin-top:6px;border-top:1px solid var(--di-border-light, #eee);padding-top:6px;display:flex;flex-direction:column;gap:2px;">
